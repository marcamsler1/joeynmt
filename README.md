# Our changes to this repo (for Exercise 3):
To resume the training for the post-norm model, which somehow stopped during epoch 5, we had to make an adjustment to the make_model_dir function in the joeynmt/helpers.py file.

We also had to replace the symlink_update function in the same file, since it generated an error message on our device (Windows) which interrupted the training. The new function was generated by ChatGpt.

Finally, we had to make a slight adjustment to the save_checkpoint method in the joeynmt/training.py file, which also caused an error and therefore interrupted training.
